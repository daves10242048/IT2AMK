	...
	class Converter
	{
		...
		private static void _run_brr_converter(string wav_file, string brr_file, int loop_point)
		{
			...
			if (config.use_snesbrr) {
				...
				if (config.filter > 0)
					filter_string = "-f" + (config.filter - 1).ToString();
				if (config.force_g)
					proc.StartInfo.Arguments = string.Format("-l{0} {1} -g {2} {3}", loop_point, filter_string, wav_file, brr_file); // f1 is only a test for now
				else
					proc.StartInfo.Arguments = string.Format("-l{0} {1} {2} {3}", loop_point, filter_string, wav_file, brr_file);
			}
