		...
		static void Main(string[] args)
		{
			...
			if (args.Length < 1) {
				...
				for (int i = 2; i < args.Length; i += 3) {
					...
					if (i + 2 < args.Length)
						conv_params.Add(Int32.Parse(args[i + 2]));
				}
			}
			...
			try {
				...
				for (int i = 0; i < it.samples.Count; i++) {
					if (it.samples[i].has_sample && (i >= use_flags.Count || use_flags[i])) {
						...
						Conv.Converter.config.force_g = (conv_params[i] % 4 == 0);
						Conv.Converter.config.use_snesbrr = (conv_params[i] % 4 == 2);
						Conv.Converter.config.filter = (conv_params[i] / 4);
